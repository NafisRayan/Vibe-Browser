...
window.addEventListener('message', (event) => {
    ...
    if (msg.command === 'updateChiiUrl' && msg.chiiUrl) {
        ...
        if (iframe && iframe.src !== msg.chiiUrl) iframe.src = msg.chiiUrl;
        return;
    }
...
function dockFrame(open: boolean) {
    const frame = getFrame();
    if (frame) {
...
function openDevToolsOverlay(chiiUrl: string) {
    let overlay = document.getElementById('vb-devtools-overlay') as HTMLDivElement | null;
    ...
    if (overlay) {
        ...
        dockFrame(willShow);
        const iframe = overlay.querySelector('iframe');
        if (iframe && iframe.src !== chiiUrl) iframe.src = chiiUrl;
        return;
    }
